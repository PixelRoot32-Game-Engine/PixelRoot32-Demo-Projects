...
// One quarter-turn, in the same 1/1024-revolution steps as kAngleSteps:
// derived rather than repeated as a literal, since it names both the
// quarter-wave table's boundary index and cosQ14's phase shift below.
constexpr uint16_t kQuarterTurn = kAngleSteps / 4;
...
int32_t sinQ14(uint16_t angle) {
    ...
    const uint16_t quadrant = static_cast<uint16_t>(wrapped / kQuarterTurn);
    const uint16_t offset = static_cast<uint16_t>(wrapped % kQuarterTurn);

    switch (quadrant) {
        case 0: return kSinQ14[offset];
        case 1: return kSinQ14[kQuarterTurn - offset];
        case 2: return -kSinQ14[offset];
        default: return -kSinQ14[kQuarterTurn - offset];  // quadrant 3
    }
}

int32_t cosQ14(uint16_t angle) {
    return sinQ14(static_cast<uint16_t>((angle + kQuarterTurn) % kAngleSteps));
}
