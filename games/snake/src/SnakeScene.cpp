...
void SnakeScene::resetGame() {
    ...
    snakeSegments.clear();
    // Reserve once so growing the snake never reallocates mid-game.
    snakeSegments.reserve(MaxSnakeSegments);
...
void SnakeScene::update(unsigned long deltaTime) {
    ...
    if (now - lastMoveTime >= (unsigned long)moveInterval) {
        ...
        if (ateFood) {
            // The pool owns every segment; snakeSegments points at the active
            // ones. Growing means claiming the first pooled segment that is
            // not already in the active list -- no allocation, ever.
