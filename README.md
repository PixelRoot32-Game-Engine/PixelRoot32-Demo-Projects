...
| [`digital_buttons`](input/digital_buttons) | Input | The other half of `InputManager`: six pages that put the four button queries on screen at once. Its one idea is that they are four different questions — `isButtonDown` is a level, `isButtonPressed` and `isButtonReleased` are true for exactly one frame, and `isButtonClicked` is a `const` query that consumes the flag it reports, so calling it twice in a frame answers `false` the second time. Key repeat, input buffering and combo detection sit on top, in an engine-free layer with a host test suite. |
...
| [`scene_transitions`](graphics/scene_transitions) | Graphics | Two plain scenes and a catalogue of transitions: fade, iris with default and corner centres, and a diagonal wipe in all four directions. Its one idea is that a transition is a full-screen post-effect applied during the swap. It belongs to neither scene, and neither scene knows it exists. The README also explains how the engine, not the demo, chooses between the 8bpp and RGB565 code paths. |
